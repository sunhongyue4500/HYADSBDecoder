 ...
 *  DF19 --- AF
 *
 *  @param bufp dataPointer
 *
 *  @return seperate the 6th - 8th bit
 */
 ...
 * ------------------------------------------------------------------------ */
unsigned long int RemPar(unsigned char *dfp, unsigned long int *r, int nb)
{
   ...
   *r = 0;
   ...
   br = nb-25;				/* shift count for remainder 如果nb = 112 ,br = 87*/
   bl = (nb/8)-3;			/* buffer length in bytes */   //if nb = 112 bl=11
   bs = bl+3;				/* bytes to shift */   //if nb = 112 bs=14
   
/* fill internal buffer */
    // ------如果bl = 11，将消息的前11个字节，存到buf[3...13]中
   for (n=0; n<bl; n++)	buf[n+3] = *(dfp+n);
   
/* polynom division 多项式除法 算法？*/
    // ------br = 87
    for (n=0; n<=br; n++)
    {
...
/* return last 24 bits of buffer */
   if (nb == 56) cs = SepAP56(dfp);
   else cs = SepAP112(dfp);
   return cs;
...
/* 
 * fetch surveillance status from DF17 ME 'airborne position message' 获得监视状态
 * 	ME bits 6..7 (2)
 * */
...
/* 
 * fetch nic supplement b  from DF17 ME 'airborne position message' 获取单天线
 * 	bit 8 (1)
 * */
 ...
 *  6bit * 8 = 48bit 即6个字节，分成两段来处理， 每三个字节有4个6bit (参见ADSB标准P42)
 * ------------------------------------------------------------------------
 * */
...
int MB2Ident(unsigned char *b, char *s)
{
    ...
    MB2Ident_2(b+3, s+4);		/* next 3 bytes */
...
/* common to all subtypes  共有的字段*/

   /* some flags */
    //垂直速度源 和ME的第36与，取得垂直速度源
   if (*(b+4)&0x10 != 0) p->verts = 1; /* vertical speed source (bit 36) */
 ...
 * 
 * return 0 on success
 *        -1 time difference odd/even too long  超时，超过10s,认为接收到这一帧距离上一帧比较久远
 *        -2 NL(rlat) == 0 error
 *        -3 NL(rlat0/1) error
 * ------------------------------------------------------------------------
 */
...
{
    int i;
    //get CPR format even or odd
    i = FetchCPRF(d);
    c->cprf = i;
    c->t[i] = time(NULL);
    //对相应的帧(odd/even)进行复制
    c->yz[i] = FetchELat(d);     /*得到编码纬度*/
    c->xz[i] = FetchELon(d);
    //Add
    return -1;
}
